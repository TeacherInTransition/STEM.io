    ...
    <div className="min-h-screen p-6 md:p-12 font-sans text-[var(--ink)] transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        ...
        <header className="mb-8 md:mb-12">
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-2 font-serif">STEM Dashboard</h1>
        ...
        <div className="flex flex-col gap-8 mb-12 w-full">
          {publishedLessons.length > 0 && (
            <div className="flex flex-col w-full">
              <div 
                onClick={() => setSelectedTrackId(selectedTrackId === 'published' ? null : 'published')}
                className={`flex flex-row items-center gap-6 p-4 rounded-[24px] cursor-pointer transition-all duration-300 border border-transparent ${
                  selectedTrackId === 'published' ? 'bg-transparent' : 'hover:bg-[var(--surface)]/40'
                }`}
              >
                <div className={`w-20 h-20 rounded-[20px] flex items-center justify-center shrink-0 ${selectedTrackId === 'published' ? 'bg-[var(--paper-2)] text-[var(--amber)] shadow-sm ring-1 ring-[var(--amber)]' : 'bg-[var(--paper-2)] text-[var(--amber)]'}`}>
                  <BookOpen className="w-8 h-8" />
                </div>
                
                <div className="flex-1 min-w-0 pt-1">
                  <h3 className="font-serif font-bold text-2xl text-[var(--ink)] mb-1.5 truncate">Published Lessons</h3>
                </div>
                      ...
                      >
                        ...
                        <div className="w-[160px] h-[160px] bg-[var(--surface)] rounded-[24px] shadow-sm border border-[var(--line)] flex flex-col items-center justify-center relative transition-all hover:shadow-md group-hover:border-[var(--amber)]">
                          <div className="absolute top-4 left-4 px-2.5 py-1 bg-[var(--paper)] text-[var(--muted)] text-[10px] font-mono rounded-md border border-[var(--line)] shadow-sm">
                            <span className="text-[var(--amber)] font-bold">L{i+1}</span>
                          </div>
