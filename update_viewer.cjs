const fs = require('fs');
let content = fs.readFileSync('src/components/LessonViewer.tsx', 'utf8');
...
content = content.replace(/return \(\s*<div className="flex flex-col min-h-screen bg-\[#090d16\][\s\S]*?\);\s*\}/m, newReturn + '\n}');
fs.writeFileSync('src/components/LessonViewer.tsx', content);
